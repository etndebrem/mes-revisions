/* ===== QCM type annale (C. Drand, janv. 2023) — logique ===== */
(function () {
  "use strict";

  var BANK = (window.QCM || []).slice();
  var THEMES = window.QCM_THEMES || [];
  var SECONDS_PER_Q = 72;            // annale : 50 questions en 1 h
  var STORE_KEY = "histdroit_qcm_v1"; // stats { id: {seen, ko} }
  var LETTERS = ["A", "B", "C", "D"];
  ...
  /* --- réglages de session --- */
  var setup = { themes: {}, count: 50, mode: "examen", chrono: true, onlyMissed: false, bareme: "2023" };
  ...
  function renderSetup() {
    ...
    var counts = [10, 20, 50, 0]; // 0 = tout
    var countBtns = counts.map(function (n) {
      var label = n === 0 ? "Tout" : (n === 50 ? "50 (annale)" : String(n));
      ...
      '<div class="controls" style="box-shadow:none;border:0;padding:0;margin:0">' +
        ...
        '<div class="row" style="margin-top:12px"><span class="filterlabel">Barème</span>' +
          '<div class="seg" id="qcm-bareme">' +
            '<button data-bareme="2023" class="' + (setup.bareme === "2023" ? "on" : "") + '">2023 (sans réponse : 0)</button>' +
            '<button data-bareme="2019" class="' + (setup.bareme === "2019" ? "on" : "") + '">2019 (sans réponse : −0,5)</button>' +
          '</div></div>' +
        '<label class="opt" id="qcm-chrono-row"><input type="checkbox" id="qcm-chrono"' + (setup.chrono ? " checked" : "") + '> Chronomètre (1,2 min par question, comme à l’annale)</label>' +
        (nMissed > 0
  ...
  function sessionScore() {
    ...
    return { ok: ok, ko: ko, skip: skip, points: ok - ko * 0.5 - skip * skipPenalty() };
  }
    ...
    $("#qcm-stage").innerHTML =
      ...
      '<div class="card qcm-card">' +
        '<div class="qcm-badges"><span class="badge theme">' + esc(it.q.th) + '</span>' +
          (it.q.an ? '<span class="badge annale" title="Question posée à cette annale (sous une forme proche)">📜 annale ' + esc(anLabel(it.q.an)) + '</span>' : '') + '</div>' +
      ...
      '<div class="qcm-foot">' +
        ...
        '<span class="qcm-hint">' + (run.mode === "entrainement" ? 'Répondez avec <span class="kbd">A</span> <span class="kbd">B</span> <span class="kbd">C</span>' : 'Barème : +1 / −0,5 / ' + (skipPenalty() ? "−0,5" : "0")) + '</span>' +
  ...
  function answer(idx) {
    ...
    var verdict = idx === null
      ? "⏭ Sans réponse (" + (skipPenalty() ? "−0,5 pt" : "0 pt") + ")"
      : (it.opts[idx].good ? "✅ Bonne réponse (+1 pt)" : "❌ Mauvaise réponse (−0,5 pt)");
    $("#qcm-expl").innerHTML =
  ...
  function finishRun(aborted) {
    ...
    // stats persistées (les questions non répondues comptent comme ratées si abandon ? non : ignorées)
    run.items.forEach(function (it) {
    ...
    d.innerHTML =
      ...
      '<p style="color:var(--muted)">soit <b>' + noteStr + ' / 20</b> au barème ' + setup.bareme + ' (+1 / −0,5 / ' + (skipPenalty() ? "−0,5" : "0") + ')<br>' +
  ...
  document.addEventListener("keydown", function (ev) {
    ...
    var idx = { a: 0, b: 1, c: 2, "1": 0, "2": 1, "3": 2 }[k];
    var it = run.items[run.pos];
