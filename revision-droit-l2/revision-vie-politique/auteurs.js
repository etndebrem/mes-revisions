...
(function () {
  ...
  function duelTable(a, b, cle) {
    // union ordonnée des rubriques présentes chez l'un ou l'autre
    var order = ["Thèse", "Concept", "Apport", "Citation", "Rôle", "Figure-type", "Œuvre", "Contexte"];
    var themes = [];
    ...
    function cell(p, t) {
      var matches = p.pos.filter(function (x) { return x.t === t; });
      if (!matches.length) return '<span class="aut-na">—</span>';
      return matches.map(function (m) { return esc(m.p); }).join('<br><br>');
    }
  ...
  function render() {
    ...
    if (state.duel !== null) {
      ...
      nItems = items.length;
      // groupés par catégorie, dans l'ordre AUTEUR_GROUPES
    ...
    c.innerHTML =
      '<div class="controls">' +
        ...
        '</div></div>' +
        (state.duel === null ?
          '<div class="row" style="margin-top:10px"><input type="search" id="aut-search" placeholder="🔎 Rechercher un penseur, un concept, une citation…" value="' + esc(state.search) + '"></div>' +
          '<div class="row" style="margin-top:10px"><span class="filterlabel">Catégorie</span><div class="chipset">' + groupChips +
            '<button class="fchip" id="aut-reset">✕ toutes</button></div></div>'
          : '') +
      '</div>' +
      (state.duel === null ? '<p class="count"><span id="aut-count">' + nItems + '</span> personnage(s)</p>' : '') +
      body;
