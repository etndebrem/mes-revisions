/* ===== QCM — Droit commercial (T. de Ravel d'Esclapon) =====
   Moteur repris du deck droit-ue. Particularité : chaque question peut
   comporter UNE OU PLUSIEURS bonnes réponses (cases à cocher). Le point
   n'est acquis que si la sélection correspond EXACTEMENT à l'ensemble des
   bonnes réponses. */
(function () {
  "use strict";
  ...
  var STORE_KEY = "droitco_qcm_v1";   // stats { id: {seen, ko} }
  ...
  /* --- réglages de session --- */
  var setup = { themes: {}, count: 20, mode: "examen", chrono: true, onlyMissed: false, onlyAnnale: false, bareme: "simple" };
  ...
  function renderSetup() {
    ...
    c.innerHTML =
      '<h3>QCM — Droit commercial</h3>' +
      '<p class="qcm-intro">⚠️ Comme à l\'examen oral / aux annales QCM, <b>chaque question peut avoir une OU plusieurs bonnes réponses</b> : l\'énoncé ne dit pas combien. ' +
    ...
    var annaleCb = $("#qcm-annale");
    if (annaleCb) annaleCb.addEventListener("change", function () { setup.onlyAnnale = this.checked; updateN(); });
  ...
  function updateN() { var n = $("#qcm-n"); if (n) n.textContent = effectiveCount(); }
  ...
  function validate() {
    ...
    var verdict;
    if (it.picked.length === 0) verdict = "⏭ Sans réponse (0 pt)";
  ...
  document.addEventListener("keydown", function (ev) {
    var vq = $("#view-qcm");
    if (!vq || vq.style.display !== "block") return;
